...
func Test_dnsHelper_setEndpoints(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			s := dnsHelper{Client: f}
			if err := s.setEndpoints(context.TODO(), testCase.mcgTarget, testCase.dnsRecord, testCase.listener, v1alpha1.LoadBalancedRoutingStrategy, "clusterid"); (err != nil) != testCase.wantErr {
