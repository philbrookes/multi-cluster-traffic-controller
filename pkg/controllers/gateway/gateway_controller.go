...
func (r *GatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if isDeleting(upstreamGateway) {
		log.Info("Gateway is deleting", "gateway", upstreamGateway.Name, "namespace", upstreamGateway.Namespace)
	...
	if !reflect.DeepEqual(upstreamGateway, previous) {
		err = r.Update(ctx, upstreamGateway)
		if err != nil {
	...
	if !isDeleting(upstreamGateway) && !reflect.DeepEqual(upstreamGateway.Status, previous.Status) {
		if err := r.Status().Update(ctx, upstreamGateway); err != nil {
			log.Error(err, "Error updating Gateway status")
...
func (r *GatewayReconciler) reconcileDownstreamGateway(ctx context.Context, gateway *gatewayv1beta1.Gateway, params *Params) (bool, metav1.ConditionStatus, []string, error) {
	...
	downstream.Labels["kuadarant.io/managed-gateway"] = "true"

	for _, s := range tlsSecrets {
		s.(*v1.Secret).ObjectMeta = metav1.ObjectMeta{
			Labels:    map[string]string{"kuadarant.io/managed-tls": "true"},
			Name:      s.GetName(),
...
func (r *GatewayReconciler) reconcileDNSEndpoints(ctx context.Context, gateway *gatewayv1beta1.Gateway, placed []string, managedHosts []v1alpha1.ManagedHost) error {
	log := log.Log
	log.Info("checking gateways for attached routes ", "clusters", placed, "managed hosts", len(managedHosts))
	if len(placed) == 0 {
		//nothing to do
		log.Info("reconcileDNSEndpoints gateway has not been placed on to any downstream clusters nothing to do")
		return nil
	}
	for _, mh := range managedHosts {
		...
		for _, downstreamCluster := range placed {
			...
			if attached == 0 {
				log.Info("no attached routes for ", "listner", listener.Name, "cluster ", downstreamCluster)
				continue
			}
			log.Info("hostHasAttachedRoutes", "host", mh.Host, "hostHasAttachedRoutes", attached)
...
func (r *GatewayReconciler) SetupWithManager(mgr ctrl.Manager, ctx context.Context) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&source.Kind{Type: &workv1.ManifestWork{}}, handler.EnqueueRequestsFromMapFunc(func(o client.Object) []reconcile.Request {
			log.Info("enqueuing gateways based on manifest work change ", "work namespace", o.GetNamespace())
			requests := []reconcile.Request{}
			annotations := o.GetAnnotations()
			if annotations == nil {
				log.Info("no parent or anotations on manifest work ")
				return requests
			}
